...
Toka 最具智慧的安全美学在于：**任何会发生状态修改的方法调用，都必须在调用点进行显式标示**。

当调用一个会修改 Shape 状态的方法时，编译器强制要求你在接收者对象（Receiver）后追加 `#` 后缀（例如 `obj#.mutate()`）。这犹如一个极高醒目度的“修改警报”，向代码阅读者与审计者最高饱和度地宣告：此处的灵魂（底层数据）正在被篡改：
```toka
{{#include ../../examples/ownership.tk:mutable_method}}
```

通过将 `#` 严格限制在“变量声明点”与“方法接收者可变调用点”，Toka 完美地在语法清爽度与高可视安全防护之间达成了伟大的工业平衡。
